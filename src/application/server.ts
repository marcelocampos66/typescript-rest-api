...
import { container, registerCoreModule, registerInfraModule, registerDomainModule } from '../core/dependency-injection';
...
(async () => {
  try {
    ...
    const rabbitMQServer = container.getInstance<BrokerServer>(BrokerInstanceTokens.RABBITMQ_SERVER);
    await rabbitMQServer.start(RABBITMQ_URI);
    const userCreatedConsumer = container.getInstance<UserCreatedConsumer>(ConsumerInstanceTokens.USER_CREATED_CONSUMER);
